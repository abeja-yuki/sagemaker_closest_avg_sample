...
class ScoringService(object):
    ...
    def get_model(cls):
        ...
        if cls.model == None:
            with open(os.path.join(model_path, 'closest_avg.pkl'), 'r') as inp:
    ...
    def predict(cls, input):
        ...
        avgs, training_params = cls.get_model()
        def finding_label(v):
            nearest_idx = (avgs[training_params.get('feature', 'feature')] - v).abs().idxmin()
            return avgs[training_params.get('label', 'label')][nearest_idx]
        return input[training_params.get('feature', 'feature')].apply(finding_label)
